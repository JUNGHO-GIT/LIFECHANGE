  ...
  try {
    ...
    const updatedEnv = linesEnv.map(line => {
      if (line.startsWith('CLIENT_URL=')) {
        return 'CLIENT_URL=https://www.junghomun.com/LIFECHANGE';
      }
      if (line.startsWith('GOOGLE_CALLBACK_URL=')) {
        return 'GOOGLE_CALLBACK_URL=https://www.junghomun.com/LIFECHANGE/api/auth/google/callback';
      }
      // 다른 줄은 그대로 유지
      return line;
    });
...
const runRemoteScript = () => {
  try {
    ...
    const cmdCd = 'cd /var/www/junghomun.com/LIFECHANGE/server';
    ...
    const cmdCh = 'sudo chmod -R 755 /var/www/junghomun.com/LIFECHANGE/server';
    const cmdStop = 'if pm2 describe LIFECHANGE >/dev/null 2>&1; then sudo pm2 stop LIFECHANGE && pm2 save; fi';
...
const restoreEnvAndIndex = () => {
  try {
    ...
    const updatedEnv = linesEnv.map(line => {
      if (line.startsWith('CLIENT_URL=')) {
        return 'CLIENT_URL=http://localhost:3000/LIFECHANGE';
      }
      if (line.startsWith('GOOGLE_CALLBACK_URL=')) {
        return 'GOOGLE_CALLBACK_URL=http://localhost:4001/LIFECHANGE/api/auth/google/callback';
      }
      // 다른 줄은 그대로 유지
      return line;
    });
