...
export const percent = async (object: any) => {
	...
	let exercise: any = {};
	exercise =
		(!object?.result?.exerciseGoal ?? !object?.result?.exercise)
			? {
					diff_count: {
						score: `1.00`,
						percent: `0.00`,
	...
	let food: any = {};
	food =
		(!object?.result?.foodGoal ?? !object?.result?.food)
			? {
					diff_kcal: {
						score: `1.00`,
						percent: `0.00`,
	...
	let money: any = {};
	money =
		(!object?.result?.moneyGoal ?? !object?.result?.money)
			? {
					diff_income: {
						score: `1.00`,
						percent: `0.00`,
	...
	let sleep: any = {};
	sleep =
		(!object?.result?.sleepGoal ?? !object?.result?.sleep)
			? {
					diff_bedTime: {
						score: `1.00`,
						percent: `0.00`,
	...
	[`exercise`, `food`, `money`, `sleep`]?.forEach((category) => {
		total.score += Number.parseFloat(
			newObject?.result?.[category]?.average.score,
		);
		total.percent += Number.parseFloat(
			newObject?.result?.[category]?.average.percent,
		);
		total.count++;
	});
