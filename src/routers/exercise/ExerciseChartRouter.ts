/**
 * @file ExerciseChartRouter.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */

import * as service from "@services/exercise/ExerciseChartService";
...
router.get(`/bar`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.bar(
...
router.get(`/bar/week`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.barWeek(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/bar/month`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.barMonth(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/pie/week`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.pieWeek(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/pie/month`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.pieMonth(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/pie/year`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.pieYear(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/line/week`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.lineWeek(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/line/month`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.lineMonth(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/avg/week`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.avgWeek(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
...
router.get(`/avg/month`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.avgMonth(
			req.query.user_id as string,
			req.query.DATE as any,
		);
		if (finalResult.status === `success`) {
