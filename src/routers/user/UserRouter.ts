...
router.post(`/email/send`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.sendEmailCode(
...
router.post(`/email/verify`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.verifyEmail(
			req.body.user_id as string,
...
router.post(`/signup`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userSignup(
			req.body.user_id as string,
			req.body.OBJECT,
		);
		if (finalResult.status === `success`) {
...
router.post(`/resetPw`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userResetPw(
			req.body.user_id as string,
			req.body.OBJECT,
		);
		if (finalResult.status === `success`) {
...
router.post(`/login`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userLogin(
			req.body.user_id as string,
...
router.get(`/detail`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userDetail(req.query.user_id as string);
		if (finalResult.status === `success`) {
			res.json({
...
router.put(`/update`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userUpdate(
			req.body.user_id as string,
			req.body.OBJECT,
		);
		if (finalResult.status === `success`) {
...
router.delete(`/delete`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.userDelete(
			req.body.user_id as string,
			req.body.user_pw as string,
		);
		if (finalResult.status === `success`) {
...
router.get(`/category/detail`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.categoryDetail(
...
router.post(`/category/update`, async (req: Request, res: Response) => {
	try {
		const finalResult = await service.categoryUpdate(
			req.body.user_id as string,
			req.body.OBJECT,
		);
		if (finalResult.status === `success`) {
