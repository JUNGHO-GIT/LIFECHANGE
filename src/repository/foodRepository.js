...
export const save = async (
  ...
  const findResult = await Food.find(filter).lean();
  ...
  if (findResult.length === 0) {
    finalResult = await Food.create(create);
  }
  else {
    finalResult = await Food.findOneAndUpdate(filter, update, options).lean();
  }
