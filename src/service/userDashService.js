...
import * as repository from "../repository/userDashRepository.js";
...
export const scatterWeek = async (
  ...
  const data = {
    "체중": {
      plan: "user_plan_weight",
      real: "user_body_weight"
    }
  };
...
export const lineWeek = async (
  ...
  for (let i = 0; i < 7; i++) {
    ...
    finalResultVolume.push({
      ...
      볼륨: intFormat(findResult?.user_total_volume || 0)
    });

    finalResultCardio.push({
      name: `${data[i]} ${dayNum.format("MM/DD")}`,
      시간: intFormat(findResult?.user_total_cardio || 0)
    });
  };
...
export const avgWeek = async (
  ...
  for (
    ...
    if (weekNum >= 1 && weekNum <= 5) {
      ...
      if (findResult) {
        sumWorkVolume[weekNum - 1] += intFormat(findResult?.user_total_volume);
        sumWorkCardio[weekNum - 1] += intFormat(findResult?.user_total_cardio);
...
export const avgMonth = async (
  ...
  for (
    ...
    if (findResult) {
      sumWorkVolume[monthNum] += intFormat(findResult?.user_total_volume);
      sumWorkCardio[monthNum] += intFormat(findResult?.user_total_cardio);
