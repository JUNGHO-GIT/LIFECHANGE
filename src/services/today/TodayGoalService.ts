...
import * as repository from "@repositories/today/TodayGoalRepository";
...
export const exist = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    finalResult = findResult.reduce((acc: any, curr: any) => {
      const curDateType = curr.today_goal_dateType;
      const curDateStart = curr.today_goal_dateStart;
      const curDateEnd = curr.today_goal_dateEnd;
