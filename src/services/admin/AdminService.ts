...
export const appInfo = async () => {
	...
	const __filename: string = fileURLToPath(import.meta.url);
	const __dirname: string = path.dirname(__filename);
	const envData: string = fs.readFileSync(path.join(__dirname, `../../../.env`), `utf8`);
	...
	const licenseRegex: RegExp = /LICENSE=(.*)/;
	...
	const licenseMatch: RegExpMatchArray | null = envData.match(licenseRegex);
	...
	const lastLicense: string = licenseMatch ? licenseMatch[1] : ``;
