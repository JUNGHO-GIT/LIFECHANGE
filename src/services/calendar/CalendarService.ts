...
export const update = async (
  ...
  if (validExerciseSection.length > 0) {
    ...
    const keepExerciseScale: string = existExerciseDetail?.result?.exercise_record_total_scale ?? `0`;
  ...
  if (validFoodSection.length > 0) {
    const foodObject = {
      ...
      food_record_total_kcal: OBJECT_param.calendar_food_record_total_calorie ?? `0`,
