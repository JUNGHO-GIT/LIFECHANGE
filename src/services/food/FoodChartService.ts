...
export const avgWeek = async (
  ...
  try {
    // promise 사용하여 병렬 처리
    const parallelResult: { findResultKcal: any[]; findResultNut: any[]; index: number }[] = await Promise.all(
    ...
    // sum, count 설정
    parallelResult.forEach(({ findResultKcal, findResultNut, index }: { findResultKcal: any[]; findResultNut: any[]; index: number }) => {
      findResultKcal.forEach((item: any) => {
