...
export const exist = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    finalResult = findResult.reduce((acc: any, curr: any) => {
      const curDateType = curr.food_record_dateType;
      const curDateStart = curr.food_record_dateStart;
      const curDateEnd = curr.food_record_dateEnd;
...
export const list = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    findResult?.sort((a: any, b: any) => {
      const dateTypeA = a.food_record_dateType;
      const dateTypeB = b.food_record_dateType;
      const dateStartA = new Date(a.food_record_dateStart);
      const dateStartB = new Date(b.food_record_dateStart);
...
export const create = async (
  ...
  // date 변수 선언
  const existingDateType = OBJECT_param.food_record_dateType;
  const existingDateStart = OBJECT_param.food_record_dateStart;
  const existingDateEnd = OBJECT_param.food_record_dateEnd;
  const dateType = DATE_param?.dateType;
  const dateStart = DATE_param?.dateStart;
  const dateEnd = DATE_param?.dateEnd;
