/**
 * @file FoodRecordChartService.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */

import * as repository from "@repositories/food/FoodRecordChartRepository";
...
export const bar = async (user_id_param: string, DATE_param: any) => {
  ...
  try {
    ...
    // goal 부재 시에도 record 기반으로 막대 생성 (goal 없으면 목표선만 빈값)
    const goalRow: any = findResultGoal?.[0];
    const recordRow: any = findResultRecord?.[0];

    finalResultKcal = [
      {
        name: String(`kcal`),
        date: String(dateStart),
        goal: String(goalRow?.food_goal_kcal ?? `0`),
        record: String(recordRow?.food_record_total_kcal ?? `0`),
      },
    ];
    ...
    finalResultNut = [
      {
        name: String(`carb`),
        date: String(dateStart),
        goal: String(goalRow?.food_goal_carb ?? `0`),
        record: String(recordRow?.food_record_total_carb ?? `0`),
      },
      {
        name: String(`protein`),
        date: String(dateStart),
        goal: String(goalRow?.food_goal_protein ?? `0`),
        record: String(recordRow?.food_record_total_protein ?? `0`),
      },
      {
        name: String(`fat`),
        date: String(dateStart),
        goal: String(goalRow?.food_goal_fat ?? `0`),
        record: String(recordRow?.food_record_total_fat ?? `0`),
      },
    ];
...
export const avgWeek = async (user_id_param: string, DATE_param: any) => {
  ...
  // sum, count 변수 선언
  let sumKcal: number[] = new Array<number>(weekCount).fill(0);
  let sumCarb: number[] = new Array<number>(weekCount).fill(0);
  let sumProtein: number[] = new Array<number>(weekCount).fill(0);
  let sumFat: number[] = new Array<number>(weekCount).fill(0);
  let countRecordsKcal: number[] = new Array<number>(weekCount).fill(0);
  let countRecordsNut: number[] = new Array<number>(weekCount).fill(0);
