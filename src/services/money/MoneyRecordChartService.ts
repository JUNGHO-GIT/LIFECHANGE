/**
 * @file MoneyRecordChartService.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */

import * as repository from "@repositories/money/MoneyRecordChartRepository";
...
export const avgWeek = async (user_id_param: string, DATE_param: any) => {
  ...
  // sum, count 변수 선언
  let sumIncome: number[] = new Array<number>(weekCount).fill(0);
  let sumExpense: number[] = new Array<number>(weekCount).fill(0);
