...
import * as repository from "@repositories/money/moneyRepository";
...
export const exist = async (
  ...
  if (!findResult || findResult.length <= 0) {
    ...
    finalResult = findResult.reduce((acc: any, curr: any) => {
      const curDateType = curr.money_dateType;
      const curDateStart = curr.money_dateStart;
      const curDateEnd = curr.money_dateEnd;
...
export const list = async (
  ...
  if (!findResult || findResult.length <= 0) {
    ...
    findResult.sort((a: any, b: any) => {
      ...
      const dateStartB = new Date(b.money_dateStart);
...
export const create = async (
  ...
  // date 변수 선언
  const existingDateType = OBJECT_param.money_dateType;
  const existingDateStart = OBJECT_param.money_dateStart;
  const existingDateEnd = OBJECT_param.money_dateEnd;
  const dateType = DATE_param.dateType;
  const dateStart = DATE_param.dateStart;
  const dateEnd = DATE_param.dateEnd;
