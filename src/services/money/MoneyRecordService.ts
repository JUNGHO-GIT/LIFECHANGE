...
export const exist = async (usrIdPrm: string, DATE_param: any) => {
	...
	if (!findResult || findResult?.length <= 0) {
		...
		finalResult = findResult.reduce(
			(acc: any, curr: any) => {
				const curDateType: any = curr.money_record_dateType;
				const curDateStart: any = curr.money_record_dateStart;
				const curDateEnd: any = curr.money_record_dateEnd;
...
export const list = async (
	...
	if (!findResult || findResult?.length <= 0) {
		...
		findResult?.sort((a: any, b: any) => {
			const dateTypeA: string = a.money_record_dateType;
			const dateTypeB: string = b.money_record_dateType;
			const dateStartA: Date = new Date(a.money_record_dateStart);
			const dateStartB: Date = new Date(b.money_record_dateStart);
...
export const detail = async (usrIdPrm: string, DATE_param: any) => {
	...
	if (!findResult) {
		...
		finalResult = findResult;
		statusResult = `success`;
		secCntRes = findResult.money_section?.length;
	}
...
export const create = async (
	...
	// date 변수 선언
	const exstDtTyp: string = OBJECT_param.money_record_dateType;
	const exstDtStrt: string = OBJECT_param.money_record_dateStart;
	const exstDtEnd: string = OBJECT_param.money_record_dateEnd;
	const dateType: string = DATE_param?.dateType;
	const dateStart: string = DATE_param?.dateStart;
	const dateEnd: string = DATE_param?.dateEnd;
