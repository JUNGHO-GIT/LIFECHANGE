/**
 * @file SleepFavoriteService.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */

import { list as listFavorite, update as updateFavorite } from "@repositories/sleep/SleepFavoriteRepository";
...
export const list = async (user_id_param: string) => {
  ...
  const finalResult: any[] = (Array.isArray(findResult) ? findResult : []).map(
    (item: any, index: number) => ({
      ...item,
      sleep_record_query: `favorite`,
      sleep_record_perNumber: index + 1,
    }),
  );
...
export const update = async (
  ...
  const favoriteKey: string = String(favorite_param?.sleep_record_key ?? ``);
  ...
  const existFavorite: boolean = favoriteList.some((item: any) => (
    item?.sleep_record_key === favoriteKey
  ));
  const nextFavorite: any[] = existFavorite
    ? favoriteList.filter((item: any) => item?.sleep_record_key !== favoriteKey)
