...
export const list = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    finalResult = await Promise.all(findResult.map(async (goal: any) => {
      ...
      // 각 평균값 구하기
      const bedTime: number = listRecord.reduce((acc: any, curr: any) => (
        acc + strToDecimal(curr?.sleep_record_bedTime)
      ), 0) / listRecord?.length;
      const wakeTime: number = listRecord.reduce((acc: any, curr: any) => (
        acc + strToDecimal(curr?.sleep_record_wakeTime)
      ), 0) / listRecord?.length;
      const sleepTime: number = listRecord.reduce((acc: any, curr: any) => (
