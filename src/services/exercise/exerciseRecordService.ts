...
export const exist = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    finalResult = findResult.reduce((acc: any, curr: any) => {
      const curDateType = curr.exercise_record_dateType;
      const curDateStart = curr.exercise_record_dateStart;
      const curDateEnd = curr.exercise_record_dateEnd;
...
export const list = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    findResult?.sort((a: any, b: any) => {
      const dateTypeA = a.exercise_record_dateType;
      const dateTypeB = b.exercise_record_dateType;
      const dateStartA = new Date(a.exercise_record_dateStart);
      const dateStartB = new Date(b.exercise_record_dateStart);
...
export const create = async (
  ...
  // date 변수 선언
  const existingDateType = OBJECT_param.exercise_record_dateType;
  const existingDateStart = OBJECT_param.exercise_record_dateStart;
  const existingDateEnd = OBJECT_param.exercise_record_dateEnd;
  const dateType = DATE_param?.dateType;
  const dateStart = DATE_param?.dateStart;
  const dateEnd = DATE_param?.dateEnd;
