...
export const list = async (
  ...
  if (!findResult || findResult?.length <= 0) {
    ...
    // goal 전체 기간을 한 번에 조회 후 메모리에서 goal별 분배 (N+1 제거)
    const rangeStart: string = findResult.reduce(
      (acc: string, curr: any) =>
        !acc || curr.exercise_goal_dateStart < acc
          ? curr.exercise_goal_dateStart
          : acc,
      ``,
    );
    const rangeEnd: string = findResult.reduce(
      (acc: string, curr: any) =>
        !acc || curr.exercise_goal_dateEnd > acc
          ? curr.exercise_goal_dateEnd
          : acc,
      ``,
    );
    ...
    finalResult = findResult.map((goal: any) => {
      ...
      // goal 기간에 속하는 record 만 메모리에서 필터 (listRecord 매칭 의미 동일)
      const listRecord: any[] = allRecord.filter(
        (curr: any) =>
          curr.exercise_record_dateStart >= dateStart &&
          curr.exercise_record_dateStart <= dateEnd &&
          curr.exercise_record_dateEnd >= dateStart &&
          curr.exercise_record_dateEnd <= dateEnd,
      );
