/**
 * @file ExerciseFavoriteService.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */

import { list as listFavorite, update as updateFavorite } from "@repositories/exercise/ExerciseFavoriteRepository";
...
export const list = async (user_id_param: string) => {
  ...
  const finalResult: any[] = (Array.isArray(findResult) ? findResult : []).map(
    (item: any, index: number) => ({
      ...item,
      exercise_record_query: `favorite`,
      exercise_record_perNumber: index + 1,
    }),
  );
...
export const update = async (
  ...
  const favoriteKey: string = String(favorite_param?.exercise_record_key ?? ``);
  ...
  const existFavorite: boolean = favoriteList.some((item: any) => (
    item?.exercise_record_key === favoriteKey
  ));
  const nextFavorite: any[] = existFavorite
    ? favoriteList.filter((item: any) => item?.exercise_record_key !== favoriteKey)
