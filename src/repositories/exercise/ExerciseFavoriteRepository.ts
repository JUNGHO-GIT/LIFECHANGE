/**
 * @file ExerciseFavoriteRepository.ts
 * @description foo
 * @author Jungho
 * @since 2025-12-26
 */
...
export const list = async (user_id_param: string) => {
  ...
  return (finalResult?.user_exercise_favorite ?? []).map(
...
export const update = async (
  ...
  const finalResult: Record<string, any> | null = await User.findOneAndUpdate(
    ...
    {
      $set: {
        user_exercise_favorite: favorite_param,
      },
    },
    {
      upsert: true,
  ...
  return finalResult?.user_exercise_favorite ?? [];
};
