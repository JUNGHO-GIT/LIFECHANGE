...
db.getCollection('sleep').insertMany(demoData, function(err, result) {
  if (err) {
    console.log(err);
  }
  else {
    db.getCollection('sleep').aggregate([
    {$group: {
      _id: "$sleep_startDt", // Group by the start date
    ...
    if (doc.deleteIds.length > 0) {
      db.getCollection('sleep').deleteMany({ _id: { $in: doc.deleteIds } });
    }
  });
}});