...
export const timeToDecimal = (data: string) => {
	...
	// 10분 단위로 반올림
	const hours = Number.parseFloat(time[0]);
	const minutes = Math.round(Number.parseFloat(time[1]) / 10) * 10 / 60;
...
export const decimalToTime = (data: number) => {
	...
	// 10분 단위로 반올림
	const floatHours = Number.parseFloat(data.toString());
...
// 4-3. combinePw ----------------------------------------------------------------------------------
export const combinePw = async (inputPw: string, token: string) => `${inputPw}_${token}`;

// 4-4. hashPw -------------------------------------------------------------------------------------
export const hashPw = async (combinedPw: string) => bcrypt.hash(combinedPw, 10);

// 4-5. comparePw ----------------------------------------------------------------------------------
export const comparePw = async (inputPw: string, storedPw: string) => bcrypt.compare(inputPw, storedPw);
