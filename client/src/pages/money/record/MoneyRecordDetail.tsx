...
export const MnyRecDtl = memo(() => {
	...
	useEffect(() => {
		if (EXIST?.[DATE?.dateType as keyof typeof EXIST]?.length > 0) {
			...
			const objectRange: string = `${OBJECT.money_record_dateStart.trim()} - ${OBJECT.money_record_dateEnd.trim()}`;
		...
		axios
			...
			.then((res: any) => {
				...
				res.data.sectionCnt > 0 &&
					setOBJECT((prev) => ({
						...prev,
						money_section: prev.money_section?.sort(
							(a: any, b: any) =>
								moneyArray.findIndex(
									(item: any) => item.money_record_part === a.money_record_part,
								) -
								moneyArray.findIndex(
									(item: any) => item.money_record_part === b.money_record_part,
								),
						),
					}));
	...
	// 2-3. useEffect ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
	useEffect(() => {
		const totals = OBJECT?.money_section.reduce(
			(acc: any, cur: any) => {
				return {
					totalIncome:
						Number(acc.totalIncome) +
						(cur.money_record_part === `income`
							? Number(cur.money_record_amount)
							: 0),
	...
	const handleDelete = useCallback((index: number) => {
		setOBJECT((prev) => ({
			...prev,
			money_section: (prev.money_section ?? []).filter(
						...
						>
							...
							<Grid container={true} spacing={1}>
								<Grid size={6} className={`d-row-left`}>
									<Bg badgeContent={i + 1} bgcolor={bgColors?.[partIndex]} />
								</Grid>
