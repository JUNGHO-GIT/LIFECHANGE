...
// -------------------------------------------------------------------------------------------------
export const MoneyFavoriteList = memo(() => {
  ...
  const recordKey: string = `money_record_key`;
  ...
  // 2-3. useEffect -----------------------------------------------------------------------------
  useEffect(() => {
    const section: any = getSession(`section`, `money`, ``) ?? [];
  ...
  const handleCheckboxChange = (index: number) => {
    ...
    const currentSection: any = getSession(`section`, `money`, ``) ?? [];
    ...
    setSession(`section`, `money`, ``, sectionArray);
  };

  // 5. format ---------------------------------------------------------------------------------
  const renderPrimaryText = (item: any): string => (
    `${translate(item.money_record_part)} / ${translate(item.money_record_title)}`
  );

  const renderSecondaryText = (item: any): string => (
    insertComma(item.money_record_amount ?? `0`)
  );
  ...
  const favoriteNode = () => {
    const listSection = () => (
      deferredObject?.map((item, i) => (
        <Grid container={true} spacing={0} key={item[recordKey] ?? `money-${i}`}>
