...
import { insertComma, setSession, getSession, sync } from "@exportScripts";
...
type FavoriteKind = `exercise` | `money` | `sleep`;

// -------------------------------------------------------------------------------------------------
export const FavoriteList = memo(() => {

  // 1. common ----------------------------------------------------------------------------------
  const {
    URL_OBJECT, PATH, firstStr, sessionId,
    location_dateType, location_dateStart, location_dateEnd,
    localCurrency, localUnit,
  } = useCommonValue();
  ...
  const sectionStorageKey: FavoriteKind = favoriteKind;
  const recordKey: string = `${favoriteKind}_record_key`;
  ...
  // 2-3. useEffect -----------------------------------------------------------------------------
  useEffect(() => {
    const section: any = getSession(`section`, sectionStorageKey, ``) ?? [];
  ...
  }, [OBJECT, PAGING.query, PAGING.page, recordKey, sectionStorageKey]);
  ...
  const handleCheckboxChange = (index: number) => {
    ...
    const currentSection: any = getSession(`section`, sectionStorageKey, ``) ?? [];
    ...
    setSession(`section`, sectionStorageKey, ``, sectionArray);
  };

  // 5. format ---------------------------------------------------------------------------------
  const renderPrimaryText = (item: any): string => {
    if (favoriteKind === `exercise`) {
      return `${translate(item.exercise_record_part)} / ${translate(item.exercise_record_title)}`;
    }
    if (favoriteKind === `money`) {
      return `${translate(item.money_record_part)} / ${translate(item.money_record_title)}`;
    }
    return `${item.sleep_record_bedTime} - ${item.sleep_record_wakeTime}`;
  };
  ...
  const favoriteNode = () => {
    const listSection = () => (
      deferredObject?.map((item, i) => (
        <Grid container={true} spacing={0} key={item[recordKey] ?? `${favoriteKind}-${i}`}>
    ...
    return (
      <Paper className={`content-wrapper radius-2 border-light-1 shadow-1 h-min-75vh`}>
        {COUNT.totalCnt === 0 ? <Empty DATE={DATE} extra={favoriteKind} /> : listSection()}
      </Paper>
    );
  };
