...
// -------------------------------------------------------------------------------------------------
export const SchedulePlannerList = memo(() => {
	...
	useEffect(() => {
		setLOADING(true);
		axios.get(`${URL_OBJECT}/planner/list`, {
			params: {
				user_id: sessionId,
				PAGING: PAGING,
				DATE: {
					...
					<ReactCalendar
						...
						tileClassName={({ date }) => {
							...
							const itemMatchesDate = (item: any) => (
								dateInRange(date, item.schedule_exercise_record_dateStart, item.schedule_exercise_record_dateEnd)
								|| dateInRange(date, item.schedule_food_record_dateStart, item.schedule_food_record_dateEnd)
								|| dateInRange(date, item.schedule_money_record_dateStart, item.schedule_money_record_dateEnd)
								|| dateInRange(date, item.schedule_sleep_record_dateStart, item.schedule_sleep_record_dateEnd)
							);
							...
							if (calendarForDates?.length > 0) {
								const sectionsCountFor = (item: any) => (
									(dateInRange(date, item.schedule_exercise_record_dateStart, item.schedule_exercise_record_dateEnd) ? (item.schedule_exercise_section?.length || 0) : 0)
									+ (dateInRange(date, item.schedule_food_record_dateStart, item.schedule_food_record_dateEnd) ? (item.schedule_food_section?.length || 0) : 0)
									+ (dateInRange(date, item.schedule_money_record_dateStart, item.schedule_money_record_dateEnd) ? (item.schedule_money_section?.length || 0) : 0)
									+ (dateInRange(date, item.schedule_sleep_record_dateStart, item.schedule_sleep_record_dateEnd) ? (item.schedule_sleep_section?.length || 0) : 0)
								);
