...
import { Schedule, ScheduleType } from "@importSchemas";
...
export const SchedulePlannerDetail = memo(() => {
	...
	const [OBJECT, setOBJECT] = useState<ScheduleType>(Schedule);
	...
	useEffect(() => {
		if (EXIST?.[DATE.dateType as keyof typeof EXIST]?.length > 0) {
			...
			const objectRange = `${OBJECT.schedule_dateStart.trim()} - ${OBJECT.schedule_dateEnd.trim()}`;
			...
			const itsNew = (
				OBJECT.schedule_dateStart === `0000-00-00` &&
				OBJECT.schedule_dateEnd === `0000-00-00`
			);
	...
	}, [EXIST, DATE.dateEnd, OBJECT.schedule_dateEnd]);
	...
	useEffect(() => {
		...
		.then((res: any) => {
			setLOADING(false);
			setOBJECT(res.data.result || Schedule);

			// sectionCnt가 0이면 section 초기화
			if (res.data.sectionCnt <= 0) {
				setOBJECT((prev: ScheduleType) => ({
					...prev,
	...
	// 4-3. handle ----------------------------------------------------------------------------------
	const handleDelete = (index: number, section: keyof ScheduleType) => {
		setOBJECT((prev) => {
