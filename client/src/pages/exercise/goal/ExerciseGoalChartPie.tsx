...
// -------------------------------------------------------------------------------------------------
export const ExerciseGoalChartPie = memo((props: ExerciseGoalChartPieProps) => {
  ...
  // 5-1. chart ------------------------------------------------------------------------------------
  const units: Record<string, string> = { count: ``, volume: `vol` };
  const chartNode = () => {
    ...
    const formatValue = (value: number): string => {
      // cardio는 분 합계로 오므로 HH:MM 표기, scale은 사용자 단위(localUnit) 고정
      if (metric.key === `cardio`) {
        const rounded: number = Math.max(0, Math.round(value));
        const hours: string = Math.floor(rounded / 60).toString().padStart(2, `0`);
        const minutes: string = (rounded % 60).toString().padStart(2, `0`);
        return `${hours}:${minutes}`;
      }
      if (metric.key === `scale`) {
        return `${insertComma(value)} ${localUnit}`.trim();
      }
