...
export const ExerciseRecordDetail = memo(() => {
	...
	useEffect(() => {
		if (EXIST?.[DATE?.dateType as keyof typeof EXIST]?.length > 0) {
			...
			const objectRange: string = `${OBJECT.exercise_record_dateStart.trim()} - ${OBJECT.exercise_record_dateEnd.trim()}`;
	...
	// 2-3. useEffect ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
	useEffect(() => {
		const totals = OBJECT?.exercise_section?.reduce(
			(acc: any, cur: any) => {
				return {
					totalVolume:
		...
		setOBJECT((prev) => ({
			...
			exercise_record_total_cardio: `${Math.floor(totals.totalCardio / 60)
				.toString()
				.padStart(
					2,
					`0`,
	...
	const handleDelete = useCallback((index: number) => {
		setOBJECT((prev) => ({
			...prev,
			exercise_section: prev.exercise_section?.filter(
													...
													idx === i
														? {
																...
																exercise_record_title:
																	foundItem?.exercise_record_title?.[0] ?? ``,
															}
														: section,
											),
										}));
