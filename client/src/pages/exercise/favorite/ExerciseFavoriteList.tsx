...
// -------------------------------------------------------------------------------------------------
export const ExerciseFavoriteList = memo(() => {
  ...
  const recordKey: string = `exercise_record_key`;
  ...
  // 2-3. useEffect -----------------------------------------------------------------------------
  useEffect(() => {
    const section: any = getSession(`section`, `exercise`, ``) ?? [];
  ...
  const handleCheckboxChange = (index: number) => {
    ...
    const currentSection: any = getSession(`section`, `exercise`, ``) ?? [];
    ...
    setSession(`section`, `exercise`, ``, sectionArray);
  };

  // 5. format ---------------------------------------------------------------------------------
  const renderPrimaryText = (item: any): string => (
    `${translate(item.exercise_record_part)} / ${translate(item.exercise_record_title)}`
  );

  const renderSecondaryText = (item: any): string => (
    `${insertComma(item.exercise_record_set ?? `0`)} ${insertComma(item.exercise_record_rep ?? `0`)} ${insertComma(item.exercise_record_weight ?? `0`)}`
  );
  ...
  const favoriteNode = () => {
    const listSection = () => (
      deferredObject?.map((item, i) => (
        <Grid container={true} spacing={0} key={item[recordKey] ?? `exercise-${i}`}>
