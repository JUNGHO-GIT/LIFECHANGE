...
import { TodayGoal, TodayGoalType } from "@exportSchemas";
...
// -------------------------------------------------------------------------------------------------
export const TodayGoalDetail = memo(() => {
	...
	const [OBJECT, setOBJECT] = useState<TodayGoalType>(TodayGoal);
	...
	useEffect(() => {
		...
		.then((res: any) => {
			setLOADING(false);
			setOBJECT(res.data.result || TodayGoal);
	...
	// 4-3. handle ----------------------------------------------------------------------------------
	const handleDelete = useCallback((index: number, section: keyof TodayGoalType) => {
		setOBJECT((prev) => {
