...
import { Bg, Br, Div, Grid, Img, Paper } from "@exportComponents";
import {
	Count,
	Delete,
	Input,
...
export const CalendarDetail = memo(() => {
	...
	// 2-3. useRef ――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――--
	const objectRef: React.RefObject<CalendarType> = useRef(OBJECT);
	...
	useEffect(() => {
		if (EXIST?.[DATE?.dateType as keyof typeof EXIST]?.length > 0) {
			...
			const objectRange: string = `${OBJECT.calendar_exercise_dateStart.trim()} - ${OBJECT.calendar_exercise_dateEnd.trim()}`;
			...
			.then((res: any) => {
				...
				// sectionCnt가 0이 아니면 section 설정
					...
					setOBJECT((prev: CalendarType) => ({
						...prev,
						calendar_exercise_section:
							res.data.result?.calendar_exercise_section ?? [],
						calendar_food_section: res.data.result?.calendar_food_section ?? [],
						calendar_money_section:
							res.data.result?.calendar_money_section ?? [],
						calendar_sleep_section:
							res.data.result?.calendar_sleep_section ?? [],
					}));
				}
				// count 설정
				setCOUNT((prev) => ({
	...
	useEffect(() => {
		const totals: any = OBJECT?.calendar_exercise_section?.reduce(
			(acc: any, cur: any) => ({
				totalVolume:
		...
		setOBJECT((prev) => ({
			...
			calendar_exercise_record_total_cardio: `${Math.floor(
				totals.totalCardio / 60,
			)
				.toString()
				.padStart(
					2,
					`0`,
				)}:${(totals.totalCardio % 60).toString().padStart(2, `0`)}`,
		}));
	}, [OBJECT?.calendar_exercise_section]);

	// 2-3. useEffect (food total 계산) ―――――――――――――――――――――――――――――――――――――――――――――――――――――――――-
	useEffect(() => {
		const totals: any = OBJECT?.calendar_food_section?.reduce(
			(acc: any, cur: any) => ({
	...
	useEffect(() => {
		const totals: any = OBJECT?.calendar_money_section?.reduce(
			(acc: any, cur: any) => {
				const amount: number = Number(cur.money_record_amount);
				return {
					totalIncome:
						cur.money_record_part === `income`
							? Number(acc.totalIncome) + amount
							: Number(acc.totalIncome),
					totalExpense:
						cur.money_record_part === `expense`
							? Number(acc.totalExpense) + amount
	...
	useEffect(() => {
		const totals: any = OBJECT?.calendar_sleep_section?.reduce(
			(acc: any, cur: any) => {
				const sleepTime: any = cur.sleep_record_sleepTime?.split(`:`) ?? [
					`0`,
					`0`,
				];
				const minutes: number =
					Number(sleepTime[0]) * 60 + Number(sleepTime[1]);
				return {
		...
		setOBJECT((prev) => ({
			...prev,
			calendar_sleep_record_total_time: `${Math.floor(totals.totalTime / 60)
				.toString()
				.padStart(
					2,
					`0`,
				)}:${(totals.totalTime % 60).toString().padStart(2, `0`)}`,
		}));
		...
		section &&
			(() => {
				setOBJECT((prev: CalendarType) => ({
					...prev,
					[section]: (
						(prev[section as keyof CalendarType] as any[]) ?? []
					).filter((_item: any, idx: number) => idx !== index),
				}));
				setCOUNT((prev) => ({
					...prev,
		...
		const exerciseSection = () => (
			<>
				...
				<Grid
					container={true}
					spacing={0}
					className={`${OBJECT?.calendar_exercise_section?.length === 0 ? `radius-2` : `radius-top-2`} border-1 shadow-1 p-10px`}
				>
					<Grid size={12} className={`d-row`}>
						<Div className={`d-row-left`}>
							...
							/>
							<Div className={`fs-0-9rem fw-600`}>{translate(`exercise`)}</Div>
						</Div>
					</Grid>
				</Grid>
		...
		const foodSection = () => (
			<>
				...
				<Grid
					container={true}
					spacing={0}
					className={`${OBJECT?.calendar_food_section?.length === 0 ? `radius-2` : `radius-top-2`} border-1 shadow-1 p-10px`}
				>
					<Grid size={12} className={`d-row`}>
						<Div className={`d-row-left`}>
							<Img
								...
								className={`ml-5px mr-10px`}
							/>
							<Div className={`fs-0-9rem fw-600`}>{translate(`food`)}</Div>
						</Div>
					</Grid>
				</Grid>
																...
																idx === i
																	? {
																			...
																			food_record_carb: setNutrient(
																				section.food_record_carb,
																				`carb`,
																			) as string,
																			food_record_protein: setNutrient(
																				section.food_record_protein,
																				`protein`,
																			) as string,
																			food_record_fat: setNutrient(
																				section.food_record_fat,
																				`fat`,
																			) as string,
																		}
																	: section,
		...
		const moneySection = () => (
			<>
				...
				<Grid
					container={true}
					spacing={0}
					className={`${OBJECT?.calendar_money_section?.length === 0 ? `radius-2` : `radius-top-2`} border-1 shadow-1 p-10px`}
				>
					<Grid size={12} className={`d-row`}>
						<Div className={`d-row-left`}>
							<Img
								...
								className={`ml-5px mr-10px`}
							/>
							<Div className={`fs-0-9rem fw-600`}>{translate(`money`)}</Div>
						</Div>
					</Grid>
				</Grid>
												...
												(section: CalendarMoneySectionType, idx: number) =>
													idx === i
														? {
																...
																money_record_title:
																	(foundItem as any)?.money_record_title?.[0] ??
																	``,
															}
														: section,
											),
											...
											calendar_money_section: prev.calendar_money_section?.map(
												(section: CalendarMoneySectionType, idx: number) =>
													idx === i
														? {
																...section,
																money_record_include: e.target.checked
																	? `Y`
																	: `N`,
															}
														: section,
		...
		const sleepSection = () => (
			<>
				...
				<Grid
					container={true}
					spacing={0}
					className={`${OBJECT?.calendar_sleep_section?.length === 0 ? `radius-2` : `radius-top-2`} border-1 shadow-1 p-10px`}
				>
					<Grid size={12} className={`d-row`}>
						<Div className={`d-row-left`}>
							<Img
								...
								className={`ml-5px mr-10px`}
							/>
							<Div className={`fs-0-9rem fw-600`}>{translate(`sleep`)}</Div>
						</Div>
					</Grid>
				</Grid>
