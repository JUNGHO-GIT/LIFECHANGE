	...
	validate.current = async (
		...
		if (extra === `record`) {
			...
			const hasExercise: boolean =
				OBJECT?.calendar_exercise_section?.length > 0;
			...
			if (hasFood) {
				...
				section.forEach((_item, i) => {
					if (
						!section[i].food_record_part ||
						section[i].food_record_part === `all`
					) {
						alert(`food_record_part`, `errorFoodPart`, i);
					}
					if (
						!section[i].food_record_name ||
						section[i].food_record_name === ``
					) {
						alert(`food_record_name`, `errorFoodName`, i);
					}
					if (
						!section[i].food_record_kcal ||
						section[i].food_record_kcal === `0`
					) {
			...
			if (hasMoney) {
				...
				section.forEach((_item, i) => {
					...
					if (
						!section[i].money_record_amount ||
						section[i].money_record_amount === `0`
					) {
