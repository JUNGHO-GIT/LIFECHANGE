...
export const useCommonDate = () => {
  ...
  const createMomentWithTimezone = useCallback(
    (params?: Moment | Date | string) => (!params || params === `0000-00-00` ? moment().tz(localTimeZone) : moment(new Date(params as string)).tz(localTimeZone)),
    [localTimeZone],
  );

  const createDateFunction = useCallback((modifier?: (_m: moment.Moment) => moment.Moment) => (params?: Moment | Date | string) => {
  ...
  const createDateFunctionWithFormat = useCallback(
    (modifier?: (_m: moment.Moment) => moment.Moment) => (params?: Moment | Date | string) => {
      const m = createMomentWithTimezone(params);
