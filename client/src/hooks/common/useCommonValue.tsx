...
export const useCommonValue = (): CommonValueType => {
  ...
  return useMemo(
    (): CommonValueType => ({
      ...
      toList:
        pathParts[2] === `goal`
          ? `/${pathParts[1] ?? ``}/goal/list`
          : `/${pathParts[1] ?? ``}/record/list`,
      toDetail:
        pathParts[2] === `goal`
          ? `/${pathParts[1] ?? ``}/goal/detail`
          : `/${pathParts[1] ?? ``}/record/detail`,
      toDelete:
        pathParts[2] === `goal`
          ? `/${pathParts[1] ?? ``}/goal/delete`
          : `/${pathParts[1] ?? ``}/record/delete`,
      ...
      // API URLs
      URL_OBJECT:
        (env.VITE_APP_SERVER_URL ?? ``) +
        (env[`VITE_APP_${(pathParts[1] ?? ``).toUpperCase()}`] ?? ``),
      URL_CALENDAR:
        (env.VITE_APP_SERVER_URL ?? ``) + (env.VITE_APP_CALENDAR ?? ``),
      ...
      URL_EXERCISE:
        (env.VITE_APP_SERVER_URL ?? ``) + (env.VITE_APP_EXERCISE ?? ``),
