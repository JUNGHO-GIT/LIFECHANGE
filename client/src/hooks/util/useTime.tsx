...
export const useTime = (
  ...
  useEffect(() => {
    ...
    const getRecordSleepTime: (_section: unknown) => string | null = (section) => {
      const sectionObj: Record<string, unknown> = section && typeof section === `object` ? section as Record<string, unknown> : {};
    ...
    if (isGoalExercise) {
      ...
      const startTime: string = typeof OBJECT[`exercise_goal_dateStart`] === `string` ? OBJECT[`exercise_goal_dateStart`] : ``;
      const endTime: string = typeof OBJECT[`exercise_goal_dateEnd`] === `string` ? OBJECT[`exercise_goal_dateEnd`] : ``;
    ...
    if (!handled && isGoalSleep) {
      ...
      const bedTimeTime: string = typeof OBJECT[`sleep_goal_bedTime`] === `string` ? OBJECT[`sleep_goal_bedTime`] : ``;
      const wakeTimeTime: string = typeof OBJECT[`sleep_goal_wakeTime`] === `string` ? OBJECT[`sleep_goal_wakeTime`] : ``;
    ...
    if (!handled && isRecordSleep) {
      ...
      if (sections.length > 0) {
        const updatedSections: unknown[] = sections.map((section: unknown) => {
          ...
          const sectionObj: Record<string, unknown> = section as Record<string, unknown>;
          const bedTimeTime: string = typeof sectionObj[`sleep_record_bedTime`] === `string` ? sectionObj[`sleep_record_bedTime`] : ``;
          const wakeTimeTime: string = typeof sectionObj[`sleep_record_wakeTime`] === `string` ? sectionObj[`sleep_record_wakeTime`] : ``;
        ...
        setOBJECT((prev: Record<string, unknown>) => {
          ...
          const prevSections: unknown[] = Array.isArray(prevSectionsRaw) ? prevSectionsRaw : [];
          const isSame: boolean = prevSections.length === updatedSections.length && prevSections.every((s: unknown, idx: number) => (
            getRecordSleepTime(s) === getRecordSleepTime(updatedSections[idx])
          ));
          return isSame ? prev : ({
            ...prev,
