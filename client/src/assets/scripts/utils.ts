...
export const calcDate = (startTime: string, endTime: string) => {
	...
	const duration: Date = new Date(
		Number(end) - Number(start) + 24 * 60 * 60 * 1000,
	);
...
export const insertComma = (param: string | number) => {
	...
	const formattedNum: string = integerPart.replaceAll(
		/\B(?=(\d{3})+(?!\d))/g,
		`,`,
	);
...
export const makeForm = (
	...
	if (fileList) {
		fileList.forEach((file: File) => {
			const newFile: File = new File([file], `${Date.now()}_${file.name}`, {
				type: file.type,
			});
...
export const formatY = (
	...
	const outerCache: unknown =
		formatYCache.get(objRef) ??
		(() => {
			...
			const config = (
				type === `sleep`
					? {
							...
							topValue:
								_extra === `line` ? Math.ceil(maxValue / 100) * 100 : 24,
						}
					: type === `money`
						? (() => {
