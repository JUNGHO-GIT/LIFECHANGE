...
export const PlanList = () => {
  ...
  // 2-1. useStorage ------------------------------------------------------------------------------>
  const {val:PLAN_LIST, setVal:setPLAN_LIST} = useStorage(
    `planList(${type.typePre})`, []
  );
  const {val:planResDur, setVal:setPlanResDur} = useStorage(
    `planResDur(${type.typePre})`, "0000-00-00 ~ 0000-00-00"
  );
  const {val:planStartDay, setVal:setPlanStartDay} = useStorage(
    "planStartDay(${type.typePre})", undefined
  );
  const {val:planEndDay, setVal:setPlanEndDay} = useStorage(
    "planEndDay(${type.typePre})", undefined
  );
  const {val:planDay, setVal:setPlanDay} = useStorage(
    `planDay(${type.typePre})`, koreanDate
  );
  const {val:planPart, setVal:setPlanPart} = useStorage(
    "planPart(DAY)", "전체"
  );
  const {val:planTitle, setVal:setPlanTitle} = useStorage(
    "planTitle(DAY)", "전체"
  );
